...
if [ "${ARCH}" == "arm64" ]; then
  ...
  docker buildx bake --file ${FILE_PATH} --set *.platform=linux/x86_64 --set *.dockerfile=${DOCKERFILE_FILE_PATH}
else
  docker buildx bake --file ${FILE_PATH} --set *.dockerfile=${DOCKERFILE_FILE_PATH}
fi
